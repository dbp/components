...
let trace name = if !Wasm.Flags.trace then print_endline ("-- " ^ name)
...
let input_sexpr name lexbuf run =
  input_from (fun _ ->
    let var_opt, def = Wasm_components.Parse.parse name lexbuf Wasm_components.Parse.Module in
...
let rec run_definition def : Wasm.Ast.module_ =
  match def.it with
  | Textual m -> m
  | Encoded (name, bs) ->
    trace "Decoding...";
